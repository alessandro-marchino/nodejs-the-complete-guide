...
router.post('/', (req, res, next) => {
  ...
  const newTodo: Todo = { id: randomUUID(), text: body.text };
...
router.put('/:id', (req, res, next) => {
  ...
  const originalTodoIndex = MY_TODOS.findIndex(todo => todo.id === params.id);
  if(originalTodoIndex === -1) {
    return res.status(404).json({ message: 'Could not find todo for this id '});
  }
  MY_TODOS[originalTodoIndex] = { ...MY_TODOS[originalTodoIndex]!, text: body.text };
  res.status(200).json({ message: 'Updated todo', todos: MY_TODOS });
});

router.delete('/:id', (req, res, next) => {
  const params = req.params as RequestParams;
  MY_TODOS = MY_TODOS.filter(todo => todo.id !== params.id);
